...
def generate_data(data1, data2, data3, seq_len, pre_len, pre_sens_num):
    data = np.stack((data1, data2, data3), axis=1)
...
def load_csv(fir_dir, col, scenario):
    ...
    if scenario == "basel_data":
        file_all=['1.csv']
    if scenario == "BernPrecipitation":
        file_all=['1.csv']
    file_name = []
    for i in file_all:
